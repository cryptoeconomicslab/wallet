...
import { getTokenName, getUnitEther } from '../helpers/utils'
...
const Heading = ({
  ...
  React.useEffect(() => {
    function updater() {
      setBalanceInner(getUnitEther(tokenId, wallet.getBalance(tokenId)))
    }
  ...
  useEffectOnce(() => {
    wallet
      .syncChildChain()
      .then(() => setBalanceInner(getUnitEther(tokenId, wallet.getBalance(tokenId))))
  })
